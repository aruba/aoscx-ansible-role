...
options:
  ...
  native_vlan_tag:
    description: >
      Flag for accepting only tagged packets on VLAN trunk native,
      vlan_mode must be set to trunk.
    required: false
    type: bool
  interface_qos_schedule_profile:
    description: >
      Attaching existing QoS schedule profile to interface. *This
      parameter is deprecated and will be removed in a future version
    type: dict
    required: False
  interface_qos_rate:
    description: >
      "The rate limit value configured for
  ...
  speeds:
    description: >
      "Configure the speeds of the interface in megabits per second
  ...
  duplex:
    description: >
      "Enable full duplex or disable for half duplex (aoscx connection).
       If this value is specified, speeds must also be specified"
    type: bool
    required: false
  ...
  port_security_client_limit:
    description: >
      "Maximum amount of MACs allowed in the interface (aoscx
  ...
  port_security_macs:
    description: >
      "List of allowed MAC addresses (aoscx connection).
      Only valid when port_security is enabled"
    type: list
    required: false
  port_security_sticky_macs:
    description: >
      "Configure the sticky MAC addresses for the interface (aoscx
      connection). Only valid when port_security is enabled"
    type: dict
    required: false
  port_security_violation_action:
    description: >
      "Action to perform  when a violation is detected (aoscx
      connection). Only valid when port_security is enabled"
    type: str
  ...
  port_security_recovery_time:
    description: >
      "Time in seconds to wait for recovery after a violation
...
- name: >
    ...
    and set the VSX features to be sync'ed
  aoscx_interface:
    name: 1/1/2
    enabled: true
    vsx_sync:
...
try:
    ...
    from ansible.module_utils.aoscx_pyaoscx import get_pyaoscx_session
