...
def run_module():
    ...
    session.check_supported_firmware(ansible_module)
    if state == 'create':
        result = vsx.create_vsx(
            role,
            isl_port,
            keepalive_interface,
            keepalive_peer,
        ...
        result = vsx.delete_vsx(**session.get_session())
    else:
        ansible_module.fail_json(
            msg='No {0} state defined for VSX module.'.format(state), **result)
